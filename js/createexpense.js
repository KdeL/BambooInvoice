function init() {
	...
	datePickerString = '<label>' + lang_expense_date_issued + '<input type="hidden" name="expense_date" id="expense_date" value="' + datePicker1 + '"/></label><span id="dateIssuedDisplay">' + datePicker2 + '</span> <a href="#" id="changeDate" onclick="Effect.toggle (\'cal1Container\', \'Blind\', {duration:0.4});return false;">' + lang_invoice_change + '</a>';
...
function insert_date(cal) {
	...
	$("expense_date").value = cal.date_str('n');
...
function create_itemized_fields()
{
	...
	theTextarea.setAttribute('name', 'items['+item_count+'][item_description]');
...
{
	new Ajax.Request(base_url+'expenses/recalculate_items', {postBody: Form.serialize('createExpense'), onComplete: function(transport) {
