...
//		$invoice_number = $this->pdf($id, FALSE);
	...
	function receipt_pdf($id, $output = TRUE)
	{
		$this->lang->load('date');
		$this->load->plugin('to_pdf');
		$this->load->helper('file');
		...
		//Assumes that full payment is done in one atttempt.
		foreach($data['paymentHistory']->result() as $payment)
		{
			$data['date_last_payment'] = formatted_invoice_date($payment->date_paid);
		} 
		...
		$html = $this->load->view('invoices/receipt_pdf', $data, TRUE);
		$receipt_localized = url_title(strtolower($this->lang->line('invoice_receipt')));

		if (pdf_create($html, $receipt_localized.'_'.$data['row']->invoice_number, $output))
		{
			show_error($this->lang->line('error_problem_saving'));
		}
